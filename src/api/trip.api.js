import { TRIP_ADD, TRIP_GET_ALL, TRIP_GET_ALL_ANAYLTICS_DATA, TRIP_GET_BY_ID, TRIP_UPDATE } from "./constant";
import api from "./vehitoEnzoBaseAPI";


export const addTrip = async (data) => {
    const ownerId = sessionStorage.getItem("userId");
    const result = await api.post(TRIP_ADD.replace(':userId', ownerId), { ...data, ownerId }, {
...
export const updateTrip = async (data) => {
    const ownerId = sessionStorage.getItem("userId");
    const result = await api.put(TRIP_UPDATE.replace(':userId', ownerId), { ...data, ownerId, status: parseInt(data.status) }, {
        headers: { 'X-Vehito-Auth-Token': sessionStorage.getItem("issuedToken") }
    });
    return result.data;
};
...
export const getAllTripAnalyticData = async (assetId) => {
    const result = await api.get(TRIP_GET_ALL_ANAYLTICS_DATA.replace(':assetId', assetId), {
        headers: { 'X-Vehito-Auth-Token': sessionStorage.getItem("issuedToken") }
    });
    return result.data;
}